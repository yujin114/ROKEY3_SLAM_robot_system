...
class YOLOCompressedCameraNode(Node):
    def __init__(self):
        ...
        # 퍼블리셔: 압축된 이미지 + 상태
        self.image_publisher_ = self.create_publisher(CompressedImage, '/robot0/cam/image/compressed', 10)
        self.status_publisher_ = self.create_publisher(Int32, '/robot0/status', 10)
        ...
        while self.running and rclpy.ok():
            ...
            if object_detected:
                if self.current_status == 1:
                    ...
                    self.get_logger().info("Car detected again. Published 0 to /robot0/status")
            ...
            if self.detected_once and not object_detected:
                if (current_time - self.last_detect_time > 3.0) and self.current_status == 0:
                    ...
                    msg.data = 1
                    self.status_publisher_.publish(msg)
                    self.get_logger().info("No detection for 3 seconds. Published 1 to /robot0/status")
