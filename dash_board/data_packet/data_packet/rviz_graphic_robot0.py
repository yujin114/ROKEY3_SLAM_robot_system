...
class MapOverlayDashboard(Node):
    def __init__(self):
        ...
        self.create_subscription(OccupancyGrid, '/robot0/map', self.cb_map, 10)
        self.create_subscription(Path, '/robot0/plan', self.cb_plan, 10)
        self.create_subscription(OccupancyGrid, '/robot0/global_costmap/costmap', self.cb_costmap, 10)
        self.create_subscription(PoseStamped, '/robot0/amcl_pose', self.cb_pose, 10)  
        self.create_subscription(PointStamped, '/robot0/detected_object_map_point', self.cb_detected_obj, 10)

        self.publisher = self.create_publisher(CompressedImage, '/robot0/map_dashboard/compressed', 10)
    ...
    def render(self):
        ...
        if rx is not None and ry is not None:
            if 0 <= rx < w*2 and 0 <= ry < h*2:
                cv2.circle(map_img, (rx, ry), 4, (0, 0, 255), -1)
        ...
        # === ROS 이미지로 Publish ===
        ret, buf = cv2.imencode('.jpg', map_img, [int(cv2.IMWRITE_JPEG_QUALITY), 80])
        if ret:
            msg = CompressedImage()
